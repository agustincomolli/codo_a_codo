...
def insert_person(name: str, age: int):
    ...
    try:
        ...
        sql_insert = f"INSERT INTO railway.persons (name, age) VALUES ('{name}', {age});"
        cursor.execute(sql_insert)
        conection.commit()  # se guardan los cambios
    except Exception as error:
        # Imprime cualquier error que ocurra durante el proceso
        print(color_text("ERROR:", COLORS.RED), f"No se ha podido insertar el registro.\n{error}")
    finally:
...
def delete_person(id: int):
    ...
    try:
        ...
        print(color_text("ERROR:", COLORS.RED), f"No se ha podido borrar el registro.\n{error}")
    finally:
        conection.close()  # Cierra la conexión a la base de datos
...
def update_person(id: int, name: str, age: int):
    ...
    try:
        ...
        sql_update = f"UPDATE railway.persons SET name = '{name}', age = {age} WHERE id = {id};"
        cursor.execute(sql_update)
        conection.commit()  # Se guardan los cambios en la base de datos
    except Exception as error:
        # Se imprime cualquier error que ocurra durante el proceso
        print(color_text("ERROR:", COLORS.RED), f"No se ha podido actualizar el registro.\n{error}")
    finally:
        conection.close()  # Cierra la conexión a la base de datos
...
def read_persons():
    ...
    try:
        ...
        # Se imprime cualquier error que ocurra durante el proceso
        print(color_text("ERROR:", COLORS.RED), f"No se ha podido leer los registros.\n{error}")
    finally:
        conection.close()  # Cierra la conexión a la base de datos

def create():
    """
    Esta función se usa para obtener los detalles de una nueva persona del usuario y guardarla en la base de datos.
    """
...
def update():
    """
    Esta función se utiliza para obtener del usuario los detalles actualizados de una persona y guardarlos en la base de datos.
    """
    id = int(input_color("Id: ", COLORS.DEFAULT, COLORS.CYAN))
    name = input_color("Nombre: ", COLORS.DEFAULT, COLORS.CYAN)
    age = int(input_color("Edad: ", COLORS.DEFAULT, COLORS.CYAN))
    update_person(id, name, age)  # Almacena los nuevos detalles en la base de datos

def delete():
    """
    Esta función se utiliza para obtener del usuario el ID de una persona y borrarla de la base de datos.
    """
    id = int(input_color("Id: ", COLORS.DEFAULT, COLORS.CYAN))  # obtiene el id del usuario
    delete_person(id)  # borra a la persona con el id correspondiente de la base de datos
...
def view_all():
    ...
    for person in persons:
        print(person)  # imprime los datos de cada persona
    input("Presione ENTER para continuar...")  # espera a que el usuario presione enter para continuar

# El bucle infinito se mantiene hasta que el usuario decida salir 
menu = 0
while menu != 5:
    clear_screen()  # limpia la pantalla
    print_title("Bases de datos\n", COLORS.YELLOW)  # imprime el título
    ...
    print("\t5 - Salir")
    menu = int(input_color("\nIngrese una opción: ", COLORS.DEFAULT, COLORS.CYAN))
